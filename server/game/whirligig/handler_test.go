...
func TestGame(test *testing.T) {
	...
	commands := []Command{
		{"next", map[string]any{"from": "start"}},
		{"next", map[string]any{"from": "intro"}},
		{"next", map[string]any{"from": "questions"}},
		{"next", map[string]any{"from": "question_whirligig"}},
		{"next", map[string]any{"from": "question_start"}},
	}
	...
	_, err = game.ProcessCommand("next", map[string]any{"from": "question_discussion"})
	assert.Nil(test, err)
	...
	_, err = game.ProcessCommand("next", map[string]any{"from": "question_discussion"})
	_, err = game.ProcessCommand("next", map[string]any{"from": "answer"})
	assert.Nil(test, err)
	...
	_, err = game.ProcessCommand("next", map[string]any{"from": "right_answer"})
	...
	commands = []Command{
		{"next", map[string]any{"from": "question_whirligig"}},
		{"next", map[string]any{"from": "question_start"}},
		{"next", map[string]any{"from": "question_discussion"}},
		{"next", map[string]any{"from": "answer"}},
		{"answer", map[string]any{"correct": false}},
		{"next", map[string]any{"from": "question_end"}},
